...
namespace blink_logger {
    ...
    //
    // Используется:
    // - для применения значений из конфига
    // - - при условии, что конфиг загрузился
    // - - значения по умолчанию: cli_enabled = false; debug_enabled = true;
    // - - - для того чтобы независимо от конфига, cli не засорялся, 
    // - - - а информация всегда печаталсь в лог
